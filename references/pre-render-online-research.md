...
- 如果 `facts-workspace.json` 中本次行程的候选城市和每个 `route_places` 地点已经有明确的 `elevation_m` 数值，且对应地点/城市的 `conflicts`、`notes` 或相关全局提醒中没有海拔数值冲突、来源冲突或“待确认”提示，则视为海拔信息已充分，不再联网查询海拔。
- 如果只有部分城市或地点已有海拔，或已有海拔无法覆盖本次行程中所有候选城市和 `route_places` 地点，则只查询缺失或不确定的目标；不要重复查询已经明确且无冲突的目标。
- 如果 facts 中已有海拔信息彼此矛盾，或冲突会影响是否属于高海拔、是否需要高反提醒、衣物准备或路线判断，才允许联网核验冲突目标。

允许查询：

- 为判断是否触发规则，可以查询行程城市或疑似高海拔景点的海拔。
- 一旦确认任一行程城市或地点海拔大于 2000 米，必须确保本次行程中每个候选城市和每个 `route_places` 地点都有明确海拔；优先复用 facts 中已明确且无冲突的海拔，只联网查询缺失或冲突的目标。
