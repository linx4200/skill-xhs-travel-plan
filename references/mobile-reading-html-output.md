...
- `assets/photos/路线地点名/`：从输入材料文件夹复制而来的本地照片；没有照片时不创建空目录。
...
每日详情页中的照片只放在“当日景点详细信息”的对应 `route_places` 地点下面。首页不放照片，城市详情页不复制每日景点照片。若县城老城、街区、古镇或观景台已经进入 `route_places`，且输入材料中有可归属照片，也应在该地点详情下展示。
...
输出目录中的照片按 `route_places` 地点名组织，不按 Day 组织。地点是素材归属，Day 只是本次路线编排。

### 展示位置

- 首页不展示照片。
- 城市详情页不复制每日景点照片。
- 每日详情页中，照片只放在“当日景点详细信息”的对应 `route_places` 地点下面。
- 入口、停车场、观景点、路线提示等执行型照片，也放在对应景点下。

### 数量与跳过

- 每个景点默认展示 1-3 张相关照片。
- 如果某个 `route_places` 地点没有对应照片，直接跳过照片区域；如果有可归属本地照片，不要因为它是县城、街区、古镇或观景台而跳过。
