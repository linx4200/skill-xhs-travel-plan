...
| `--alert` | `#b42318` | 超过 2500 米的高海拔提醒色，仅用于 `.elevation-alert` |
| `--measure` | `680px` | 移动端优先的正文最大宽度 |

配色以中性灰白为主。链接蓝色只用于可点击文本；`--alert` 红色只用于超过 2500 米的海拔提醒；折叠箭头、timeline 主线和节点都只表示结构或交互状态，不用于装饰。
...
```
- 照片只作为本地展示素材；攻略事实和照片说明不得来自图片画面解析，只能来自文本材料、照片目录名和文件名。
...
- `alt` 和 `figcaption` 只能根据地点名、目录名、文件名或文本材料中已确认的信息生成；不能打开或视觉解析图片来描述画面内容。

### 5.8 关键约束
- 标题必须放在 `<summary>` 内（h2/h3），否则不会获得折叠箭头与悬停态。
- `summary` 内只放一个标题元素（h2 或 h3），不要塞段落。
- 基础阅读结构不要为了样式额外加 `class` 或 `id`。仅允许使用 `.timeline`、`.page-nav`、`.meta`、`.photo`、`.photo-grid`、`.elevation-alert` 这类有明确语义和功能的 class。
...
- 高海拔提醒：`elevation_m > 2500` 的海拔列表项可使用 `.elevation-alert` 和 `--alert` 红色；不要把该红色扩展到普通标题、卡片或装饰元素。
