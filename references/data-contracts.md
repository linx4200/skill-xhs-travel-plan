...
- `days[].title`：当天标题，概括城市、主要路线和当天住宿城市；只要 `lodging_city` 非空，标题必须包含该城市，例如 `2026-10-03 - 毕节 - 九洞天 - 宿毕节`。
...
- `day`
- `date`
- `title`：当天标题，必须沿用 `route-structure.json.days[].title` 的同一要求；只要 `lodging_city` 非空，标题中必须包含该城市。
- `lodging_city`
- `summary`
- `route_places`
- `timeline`
